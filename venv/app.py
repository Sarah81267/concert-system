...
from database import db, User, Concert,Seat,Feed_back  # Import your database module
...
@app.route('/')
def home():
    concerts = Concert.query.all()  # Fetch all concerts from the database
...
def feedback():
    ...
    if request.method == 'POST':
        ...
        if not concert_id or not rating:
            flash('Please select a concert and provide a rating.')
        ...
        if not concert:
            flash('Selected concert not found.')
            return redirect(url_for('feedback'))
    ...
    return render_template('feedback.html', concerts=concerts)

@app.route('/thank_you')
def thank_you():
    return render_template('thank_you.html')

@app.route('/ticket_confirmation/<int:seat_id>', methods=['GET'])
def ticket_confirmation(seat_id):
    seat = Seat.query.get_or_404(seat_id)
    concert = Concert.query.get_or_404(seat.concert_id)  # Get the concert associated with the seat
    
    # Pass the seat and concert information to the template
    return render_template('ticket_confirmation.html', seat=seat, concert=concert)
...
def pay(concert_id, ticket_type):
    ...
    if ticket_price == 0:
        ...
        if available_seat:
            ...
            available_seat.user_id = session.get('user_id')  # Assuming the user is logged in
            
            db.session.commit()  # Commit the seat status change
            
            # Redirect to ticket confirmation with seat_id
            flash("Ticket has been successfully reserved for free!", 'success')
            return redirect(url_for('ticket_confirmation', seat_id=available_seat.id))  # Pass seat_id here
        else:
...
def payment_execute(concert_id, ticket_type):
    # Retrieve PayPal payment details from the URL
    payment_id = request.args.get('paymentId')
    payer_id = request.args.get('PayerID')
    
    # Ensure the payment and payer IDs are present
    if not payment_id or not payer_id:
        flash('Invalid payment details. Please try again.', 'error')
        return redirect(url_for('tickets', concert_id=concert_id))
    
    # Find the payment using the payment_id
    payment = paypalrestsdk.Payment.find(payment_id)
    
    # Attempt to execute the payment if found
    if payment.execute({"payer_id": payer_id}):
        ...
        if available_seat:
            ...
            if not user_id:
                flash('You need to log in to complete the purchase.', 'error')
                return redirect(url_for('login'))  # Redirect to login if no user session

            # Update seat status to 'sold' and associate it with the user
            available_seat.status = 'sold'
            available_seat.user_id = user_id
            db.session.commit()  # Commit the seat update
            
            # Get the concert information and update the sold ticket count
            concert = Concert.query.get(concert_id)
            if concert:
                if ticket_type == 'VIP':
                ...
                db.session.commit()  # Commit the concert update
            
            flash('Payment successful! Your ticket has been purchased.', 'success')
        else:
            flash('No available seats for the selected ticket type.', 'error')
        
        # Redirect to a confirmation page showing the ticket details
        return redirect(url_for('ticket_confirmation',seat_id=available_seat.id))
...
@app.route('/cancel')
def payment_cancel():
    ('Payment canceled.', 'warningflash')
    return redirect(url_for('concerts'))

if __name__ == '__main__':
    with app.app_context():
        db.create_all()  # Create database tables if they don't exist
    app.run(host='0.0.0.0', port=int(os.environ.get('PORT', 5000))) # Run the Flask app
